...
### 捐赠接口 `/api/v1/donations`
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| POST | `/` | 登录 | 发起捐赠 |
| GET | `/my` | 登录 | 我的捐赠记录 |
| GET | `/` | 管理员 | 所有捐赠记录 |
| GET | `/statistics` | 管理员 | 捐赠统计 |
...
- [x] 后端：我的机构/审核（管理员）
- [x] 后端：组织注册申请（公开接口，支持资历图片上传）
- [x] 前端：机构审核页面（管理员）
- [x] 前端：组织注册页面（用户/组织选择、资历图片上传）

**捐赠模块**
- [x] 后端：发起捐赠/我的捐赠/捐赠列表/统计（管理员）
- [x] 前端：捐赠页面（捐赠表单、金额选择）
- [x] 前端：我的捐赠页面（捐赠记录查看）
- [x] 前端：捐赠导航链接（Header、Footer）
...
**部署配置**
- [x] Docker 完整配置（前端 + 后端 + MySQL + Redis）
- [x] Nginx 反向代理配置
- [x] 健康检查配置
- [x] Docker 部署文档

**表单验证**
- [x] 注册页面密码强度验证（大小写+数字+特殊字符）
- [x] 手机号正则验证（中国大陆手机号）
- [x] 邮箱格式验证
- [x] 实时错误提示和字段级验证
...
- **邮箱**: tyumaoo@qq.com
- **Gitee**: https://gitee.com/olrain/pet-adoption-platform
